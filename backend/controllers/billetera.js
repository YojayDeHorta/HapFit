...
exports.update = async (req, res) => {
	try {
		...
		const actulizacion = await query(
			`UPDATE billetera SET correoPayPal = '${req.body.correoPayPal}' WHERE idBilletera='${billetera[0].idBilletera}';`
		);
		// const actulizacion = await query(
...
exports.setSolicitud = async (req, res) => {
	try {
		...
		const actulizacion = await query(
			`UPDATE billetera SET solicitud = 1 WHERE idBilletera='${billetera[0].idBilletera}';`
		);
...
exports.get = async (req, res) => {
	try {
		...
		res.json({ error: null, data: billetera[0] });
	} catch (error) {
		console.log(error);
		return res.status(400).json({ error: 'Error interno del servidor' });
	}
